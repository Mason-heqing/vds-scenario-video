<template>
  ...
  >
    ...
    <template v-for="(item,index) in tabelHeader">
      ...
      <!-- 普通 -->
      <!-- <el-table-coumn v-else-if="item.secHeader" v-for="(list,idx) in item.secHeaderData" :key="idx" :label="list.orderDay">
            <el-table-column label="专线包" width="320" :resizable="false">
            <template slot-scope="scope">
      ...
      <el-table-coumn  v-else-if="item.secHeader" label="时间">
           <el-table-column label="专线包" prop="specialTotalPrice" align="center" sortable="custom" width="320" :resizable="false">
            <template slot-scope="scope">
               <span>{{ scope.row[item.param] }}</span>
            </template>
          </el-table-column>
          <el-table-column label="激活包" prop="activateTotalPrice" align="center" sortable="custom" width="320" :resizable="false">
            <template slot-scope="scope">
               <span>{{ scope.row[item.param] }}</span>
            </template>
          </el-table-column>
  ...
  watch: {
    tableLabel: {
      handler(newValue) {
        ...
        console.log("头部数据:",this.tabelHeader);
      },
      deep: true
    }
  },
