...
export default {
  ...
  created() {
    ...
    // let token = '5cbb74491c6d45f78daecdc06e35485f'
    let isMaster = this.getParam("isMaster");
    // let isMaster = 'false';
    let orgId = this.getParam("orgId");
    let orgId = '00a1124f77334edeb70beff0a60eca30';
    if (token) {
      Cookies.set("TOKEN", token);
      this.$router.push("/");
    }
    if(isMaster){
      Cookies.set("isMaster",isMaster);
    }
    if(orgId){
          ...
          if (res.code == 200) {
            if ("" != res.data.logMinUrl) {
              if (-1 == res.data.logMinUrl.indexOf("http")) {
                this.logMinUrl =
                  this.imgurl + "/file?path=" + res.data.logMinUrl;
              } else {
            ...
            if ("" != res.data.logUrl) {
              if(-1 == res.data.logUrl.indexOf('http')){
                  this.logUrl = this.imgurl+'/file?path='+ res.data.logUrl;
              }else{
      ...
      this.utils.http(
        ...
        (res) => {
          if (res.code == 200) {
            ...
            ligature.$emit("userId",res.data.id);
            // this.userImg = this.tools.global.API_URL + "/user/photo?id="+res.data.id;
