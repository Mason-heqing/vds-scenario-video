  ...
  <div class="content-wrapper">
      ...
      <div class="box box-primary">
        <div class="box-body">
          ...
          <table-cmp
            ...
            @sortableChange="sortableChange($event,0)"
          >
            <el-table-column
              slot="editResourceName"
              prop="editResourceName"
              label="资源池名称"
              sortable="custom"
              align="center"
              width="280"
            >
              <template slot-scope="scope">
                <div @click="editPoolName(scope.row)" style="cursor: pointer">
                  {{ scope.row.name }}
                </div>
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <div @click="editPackageSpecialized(scope.row.id)" style="cursor: pointer">
            ...
            >
              <template slot-scope="scope">
                <div
                  ...
                  :class="
                    1 === scope.row.deviceNumStatus
                      ? 'green'
                      : 2 === scope.row.deviceNumStatus
                      ? 'yellow'
                      : 'red'
                  "
                  style="width: 100%; cursor: pointer"
                >
                  {{ scope.row.deviceNumDesc }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <div
                  @click="editParallel(scope.row)"
                  :class="
                    1 === scope.row.parallelNumStatus
                      ? 'green'
                      : 2 === scope.row.deviceNumStatus
                      ? 'yellow'
                      : 'red'
              ...
              <template slot-scope="scope">
                <el-row>
                  ...
                  <el-button
                    ...
                    round
                    size="mini"
                    @click="addParallel(scope.row.id)"
                  >添加专线包</el-button>
                  <el-button
                    title="添加设备"
                    type="primary"
                    ...
                    round
                    size="mini"
                    @click="addDevice(scope.row.id, scope.row.limitDeviceNum)"
                  >添加设备</el-button>
                  <el-button
                    title="删除"
                    type="danger"
                    ...
                    round
                    size="mini"
                    @click="deleteOwner(scope.row.id)"
                  >删除</el-button>
                </el-row>
              </template>
            </el-table-column>
      ...
      >
        <div style="padding: 0px">
          <div class="row mb-20">
            ...
            <div class="col-sm-9">
              <a :href="imgurl + '/static/excel/person.xls'"
                >开始下载Excel导入模板.xls</a
              >
          ...
          <div class="row mb-20">
            ...
            <div class="col-sm-9">
              1、上传文件大小最大为：<span style="color: red">128MB</span
              >；<br />
              2、excel文件跟人员图片需要放在<span style="color: red"
                >同一层级目录</span
              >，如图片放在(批量导入目录)则excel文件也要放在该目录下；<br />
              3、目前仅支持上传<span style="color: red">zip格式</span
              >压缩文件，将文件夹压缩时请选择正确的压缩格式；<span
                style="color: red"
                >excel模板请下载上方指定的模板</span
              >；<br />
              4、人员与图片的对应方式为根据excel文件人脸图片进行获取，如果当前导入用户
              存在人脸图片，请将图片名称填入此列;<br />
              5、当人脸图片列为空时，会根据人员姓名去进行一次对应的图片获取，如人员姓名为李四，会去获取一次图片名称为李四的文件；<br />
              6、若无excel文件，仅有图片，则人脸图片名称格式为：<span
                style="color: red"
                >姓名_人员编号</span
              >
            </div>
    ...
    >
      <div style="padding: 0px">
        <div v-if="popIndex == 1">
          <h4>
            <span class="el-icon-warning icon-warning"></span>是否同意申请
          </h4>
    ...
    ></msg-box>
    <el-dialog
      :title="isCreate ? '创建专线包资源池' : '编辑专线包资源池'"
    ...
    >
      ...
      >
        <el-form-item label="资源池名称" prop="resourcesName">
          <el-input
            v-model="poolForm.resourcesName"
            placeholder="请输入资源池名称"
            maxlength="68"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="每台相机最大播放客户端数量"
          prop="limitParallelNumPerDevice"
        >
          <el-input
            v-model="poolForm.limitParallelNumPerDevice"
            onkeyup="value=value.replace(/[^\d]/g,'')"
            placeholder="请输入每台相机最大播放客户端数量"
          ></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog
      :title="isOperate ? '选择专线包添加到当前资源池' : '已绑定专线包个数'"
      :visible.sync="dedicatedPoolDialogVisible"
      width="60%"
    >
      <table-cmp
        v-loading="loading1"
        ...
        @sortableChange="sortableChange($event,1)"
      >
       <el-table-column slot="operate" label="操作" align="center" width="90" v-if="!isOperate">
                        <template slot-scope="scope">
                           <el-row>
                               <el-button title="解除资源池绑定" type="primary" icon="el-icon-s-open" round
                    size="mini" @click="noBind(scope.row)">解绑</el-button>
                            </el-row>
                        </template>
                    </el-table-column>
      </table-cmp>
    ...
    >
     ...
     <el-checkbox-group v-model="selectDevice">
        ...
        <div class="col-sm-9 pl-0">
          <div class="col-sm-4 pl-0">
            <div class="form-group">
              <div class="col-md-12">
                <el-select
                  v-model="deviceListForm.orgId"
                  filterable 
                  placeholder="选择机构"
                  @change="deviceOrgChange"
                >
                  <el-option
            ...
            <div class="form-group">
              <div class="col-md-12 pl-0">
                <el-input
                  ...
                  v-model="deviceListForm.searchStr"
                >
                </el-input>
              </div>
            </div>
      ...
      <table-cmp
        ...
        :table-label="tableLabel5"
        ref="tableChild5"
        :is-show-select="isShowSelect5"
        @sortableChange="sortableChange($event,5)"
      >
        <el-table-column slot="videoActivated" label="激活状态" align="center" width="120" sortable="custom" prop="videoActivated">
          <template slot-scope="scope">
            <img src="../../../assets/img/yes.png" title="激活状态" v-if="scope.row.videoActivated == 1" alt="已激活" width="32" height="32"/>
          </template>
        </el-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" v-if="!isBtn" @click="checkedDevide"
          >确 定</el-button
        >
        <el-button v-else type="primary" :loading="true">提交中</el-button>
      </span>
    ...
    <el-dialog
      :title="isFlag ? '已绑定设备' : '正在播放设备'"
      :visible.sync="VideoPayDialogVisible"
      width="70%"
    >
      <div class="row">
        <div class="col-sm-4">
          <el-radio-group
            v-model="resourcePoolBindedDeviceForm.playIng"
            size="medium"
            @change="radioChangeS"
            v-show="isFlag"
          >
        ...
        <div class="col-sm-8 text-right flex-style">
          <div class="form-group">
            <div class="col-md-3">
              <!-- <el-tree :props="orderDefaultProps" ref="tree" :data="orgIdList" :check-strictly="true" show-checkbox
                  node-key="orgId" default-expand-all highlight-current @check-change="orderHandleCheckChange">
                </el-tree> -->
              <el-select
                v-model="resourcePoolBindedDeviceForm.orgId"
                placeholder="选择机构"
                filterable
                style="width: 100%"
                @change="videoOrgchang"
              >
                <el-option
                  v-for="item in orgIdList"
                  :key="item.id"
            ...
            <div class="col-md-3">
              <el-select
                clearable
                v-model="resourcePoolBindedDeviceForm.appId"
                style="width: 100%"
                filterable
                placeholder="选择车场"
              >
            ...
            <div class="col-md-4">
              <el-input
                ...
                style="width: 100%"
                placeholder="请输入设备名称或序列号"
                v-model="resourcePoolBindedDeviceForm.deviceSearch"
              ></el-input>
            </div>
            <div class="col-md-2 btn-group text-right flex-style">
              <button
                id="btn_device_search"
                type="button"
                class="btn btn-primary"
      ...
      <table-cmp
        ...
        @sortableChange="sortableChange($event,2)"
      >
        ...
        <el-table-column slot="operate" label="操作" align="center" width="220">
          <template slot-scope="scope">
            <el-row>
              <!-- <i class="el-icon-edit">IC卡</i>
                             <i class="el-icon-delete">删除</i> -->
              <el-button
                ...
                round
                size="mini"
                @click="stopPush(scope.row.deviceId)"
              >停止推流</el-button>
              <el-button
                title="设备解绑"
                type="primary"
                ...
                round
                size="mini"
                @click="unbindDevice(scope.row.deviceId)"
              >解绑</el-button>
            </el-row>
          </template>
        </el-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="VideoPayDialogVisible = false">关 闭</el-button>
        <el-button v-show="isFlag" type="primary" @click="checkedDedicatedPool"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    >
          ...
          <el-tabs type="border-card" v-model="selectTab">
            <el-tab-pane label="机构" name="first">
              ...
              <el-transfer
                v-model="selectOrgId"
                :data="orgIdToDevice"
                :props="orgIdListDefaultProps"
                :titles="['可选机构', '已选机构']"
              ></el-transfer>
              <br />
              <el-checkbox v-model="containChild"
                >包含下级所有子机构</el-checkbox
              >
            </el-tab-pane>
            <el-tab-pane label="车场" name="second">
              ...
              <el-tree
                ...
                show-checkbox
                default-expand-all
                :default-checked-keys="checkedAppIds"
                @node-click="handleNodeClick"
                node-key="id"
                ref="tree"
                highlight-current
                :props="appIdDefaultProps"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="!isBtn" type="primary" @click="setDataInfo"
          >确 定</el-button
        >
        <el-button v-else type="primary" :loading="true">提交中</el-button>
      </span>
    ...
    <el-dialog
      title="播放统计展示"
      :visible.sync="videoStatisticalDialogVisible"
      width="63%"
      >
      <div class="row">
          <div class="col-lg-12">
                  ...
                  <div class="col-lg-4 pl-0">
                    <div class="form-group">
                      ...
                      <div class="col-md-12" v-else-if="2 === dataForm.operateType">
                          <el-date-picker style="height:36px;"
                      ...
                      <div class="col-md-12" v-else>
                         <el-date-picker style="width:47%;height: 36px;"
