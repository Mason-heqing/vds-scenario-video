...
export default {
  ...
  data() {
    return {
      ...
      tableLabel: [
        ...
        { label: '机构ID', param: 'orgId', align: 'left', sortable: false, isshow: false, width: '100' },
        { label: '商品ID', param: 'goodsId', align: 'left', sortable: false, isshow: false, width: '100' },
        ...
        { label: '所属资源池', param: 'bindTargetName', align: 'center',isshow:true, sortable:true,width:'120'},
	    	{ label: '激活时间', param: 'activationTimeFormat', align: 'center',isshow:true, sortable:"custom",width:'120'},
	    	{ label: '过期时间', param: 'expireTimeFormat', align: 'center',isshow:true, sortable:"custom",width:'120'},
        { label: '剩余时间', param: 'remainingSecondsFormat', align: 'center',isshow:false, sortable:"custom",width:'80'},
		    { label: '来源', param: 'sourceFormat', align: 'center',isshow:false, sortable:"custom",width:'80'},
        { label: '是否转让', param: 'transfered', align: 'center',isshow:false, sortable:"custom",width:'80',render(h) {
    ...
    orderDefaultProps: {
      ...
      filterText: '',
    mechanismList:[],//机构列表
    poolSpecializedDialogVisible:false,//选择资源池弹框
    poolSpecialized:'',//选择资源池
    poolSpecializedList:[],//资源池
	  props: {
		label: 'name',
		children: 'zones'
	  },
  ...
  watch:{
    filterText(val) {
      ...
      this.$refs.orgIdTree.setCurrentKey();
    }
  },
  components: {
    MsgBox,
  ...
  gooperation(row,type){
    ...
    this.toolType = type;
      ...
      if(this.poolSpecializedList && this.poolSpecializedList.length === 0){
        this.$message({
      ...
      this.poolSpecialized = '';
      this.poolSpecializedDialogVisible = true;
      this.getDevice();
      this.toolForm = {
        category:2,
        operateObjectId:row.bindTarget,
  ...
  getDevice(){
    ...
    res => {
      if(res.code==200){
        this.poolSpecializedList = res.data;
      }
  ...
  //选择设备
  checkedDevide(){
    if('' == this.poolSpecialized){
      this.$message.error('请选择资源池!');
    }else{
      this.toolForm.operateObjectId = this.poolSpecialized;
      this.dataSend(this.toolForm,this.toolType)
    }
  },
  ...
  dataSend(v,type){
    ...
    res => {
      ...
      if(1 ===type){
        ...
        this.poolSpecializedDialogVisible = false;
      }
      if(res.code==200){
    ...
    sortableChange( obj ) {
      ...
      // this.specialPageForm.sortProp = 'limitDeviceNum';
      // this.specialPageForm.sortOrder = 'descending';

      this.specialPageForm.propMap = {"specificationDesc":"specification","durationDesc":"duration","bindTargetName":"bindTarget"};
