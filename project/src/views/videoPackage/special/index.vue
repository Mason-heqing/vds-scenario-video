<template>
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
      <div class="box box-primary">
        <div class="box-body" v-loading="loading">
          <div class="form-horizontal">
            <div class="col-sm-2 pl-0">
              <div class="btn-group">
                ...
                <button id="btn_delete" type="button" class="btn btn-primary" @click="assignmentBtn"
                  v-show="!specialPageForm.transferFlag">
                  <span class="glyphicon glyphicon-share" aria-hidden="true"></span> 转让给其它机构
                </button>
              </div>
            </div>
          ...
          <table-cmp v-loading="loading" :table-data="tableData" :table-label="tableLabel" ref="tableChild" @sortableChange="sortableChange"
            ...
            <el-table-column slot="operate" label="操作" align="center" width="180">
              <template slot-scope="scope">
                <el-row>
                  ...
                  <el-button v-show="0 === scope.row.transfered" title="转让给其它机构" type="primary" icon="el-icon-share" round
                    size="mini" @click="gooperation(scope.row, 1)">转让</el-button>
                  <el-button v-show="0 === scope.row.binded" title="绑定到资源池" type="primary" icon="el-icon-s-check" round
                    size="mini" @click="gooperation(scope.row, 2)">绑定</el-button>
                  <el-button v-show="1 === scope.row.binded" title="解除资源池绑定" type="primary" icon="el-icon-s-open" round
    ...
    <!-- <el-dialog title="创建订单" :visible.sync="dialogVisible" width="45%">
    ...
    <el-dialog title="选择转让的目标机构" :visible.sync="mechanismDialogVisible" width="20%">
      <el-input placeholder="输入关键字进行过滤" v-if="mechanismList.length>1" v-model="filterText" class="mb-10">
        </el-input>
      <div class="device-list">
        ...
        <el-tree :props="orderDefaultProps" ref="orgIdTree" :data="mechanismList" :check-strictly="true" :filter-node-method="filterNode" @node-click="selectCurerentOrgId"
    ...
    <el-dialog title="选择资源池" :visible.sync="poolSpecializedDialogVisible" width="20%">
      <div class="device-list">
        <el-radio-group v-model="poolSpecialized">
          <el-radio-button :label="item.id" v-for="(item, index) in poolSpecializedList" :key="index">{{ item.name }}
          </el-radio-button>
        </el-radio-group>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="poolSpecializedDialogVisible = false">取 消</el-button>
    ...
    <el-dialog title="请使用微信扫码支付" :visible.sync="payDialogVisible" width="15%" class="pay-orcode">
      <div class="row">
...
.device-list /deep/.el-tree--highlight-current
