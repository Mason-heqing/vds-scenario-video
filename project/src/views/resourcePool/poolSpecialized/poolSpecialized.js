...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableData: [],
      tableLabel: [
        { label: 'id', param: 'id', align: 'left', sortable: false, isshow: false, width: '100' },
        { label: 'orgId', param: 'orgId', align: 'left', sortable: false ,isshow:false,width:'100'},
        ...
        { label: '机构数', param: 'orgNum', align: 'center', sortable:"custom",isshow:false,width:'60'},
        { label: '项目(车场)数', param: 'appNum', align: 'center',isshow:false, sortable:"custom",width:'60'},
      ...
      tableData1:[],//专线包列表
      tableLabel1: [
        { label: 'ID', param: 'id', align: 'center', sortable: false ,isshow:false,width:'120'},
        { label: '产品名称', param: 'goodsName', align: 'center', sortable:"custom",isshow:true,width:'80'},
        { label: '规格', param: 'specificationDesc', align: 'center', sortable:"custom",isshow:true,width:'80'},
        { label: '有效时长', param: 'durationDesc', align: 'center', sortable:"custom",isshow:true,width:'80'},
        { label: '可绑设备数', param: 'limitDeviceNum', align: 'center',isshow:true, sortable:"custom",width:'80'},
        { label: '购买时间', param: 'purchaseTimeFormat', align: 'center',isshow:true, sortable:"custom" ,width:'120'},
        { label: '过期时间', param: 'expireTimeFormat', align: 'center',isshow:true, sortable:"custom",width:'120'},
        // { label: '剩余时间', param: 'remainingSecondsFormat', align: 'center',isshow:false, sortable: true ,width:'80'},
		    { label: '来源', param: 'sourceFormat', align: 'center',isshow:false, sortable: true ,width:'80'},
      ],
      isOperate:false,
      ...
      tableData2:[],
      tableLabel2:[
        { label: '设备序列号', param: 'deviceSn', align: 'left',isshow:true, sortable:"custom",width:'70'},
        { label: '设备名称', param: 'deviceName', align: 'left',isshow:true, sortable:"custom",width:'80'},        
        { label: '所属机构', param: 'orgId', align: 'left', sortable:"custom",isshow:true,width:'100'},
        { label: '项目(车场)', param: 'appName', align: 'left', sortable:"custom",isshow:true,width:'80'},
        { label: '总播放时长(秒)', param: 'playSeconds', align: 'left', isshow: true, sortable:"custom", width: '65' },
        {slot:'online',isshow:true},
		    // { label: '播放状态', param: 'playing', align: 'center',isshow:true, sortable: true ,width:'80'},
        { label: '播流客户端数', param: 'playClientNum', align: 'left', isshow: true, sortable:"custom", width: '65' },
        { slot: 'operate',isshow:true }, // 操作列
      ],
      isShowSelect2:true,
      currentPage2:1,
      ...
      tableData5: [],
      tableLabel5: [
        { label: '设备名称', param: 'name', align: 'left', sortable:"custom",isshow:true,width:'80'},
        { label: '设备S/N', param: 'serial', align: 'left',isshow:true, sortable:"custom",width:'80'},
        { label: 'IP', param: 'ip', align: 'left',isshow:true, sortable:"custom",width:'80'},
        // { label: '视频流', param: 'playSeconds', align: 'center',isshow:true, sortable: true ,width:'80'},
		    { label: '软件版本', param: 'version', align: 'left',isshow:true, sortable:"custom",width:'120'},
        { label: '在线状态', param: 'online', align: 'center',isshow:false, sortable:"custom",width:'60'},
      ],
      isShowSelect5: true,
      currentPage5: 1,
      total5:0,
  ...
  methods: {
       ...
       if (valid) {
         ...
         this.utils.http({
             ...
             data: this.isCreate ? {
               ...
               limitParallelNumPerDevice:parseInt(this.poolForm.limitParallelNumPerDevice)
             } : {
               orgId: Cookies.get("orgId"),
                 poolName: this.poolForm.resourcesName,
                 limitParallelNumPerDevice:parseInt(this.poolForm.limitParallelNumPerDevice),
   ...
   addDevice(value,num){
    ...
    //  this.deviceToal = num;
     ...
     this.deviceListForm.orgId = Cookies.get("orgId");
   ...
   //选择设备
    checkedDevide() {
      ...
      if (0 === all.length) {
        ...
        selectDeviceIds = all.map(item =>{return item.id})
      }
      this.isBtn = true;
      this.utils.http({
        url: "/pool/specialized/plus/special/device",
        method: "POST",
        data: {orgId:Cookies.get("orgId"),deviceIdList:selectDeviceIds,poolId:this.resourcePoolId}
      },
        res => {
          this.isBtn = false;
        if(res.code==200){
   ...
   editPoolName(row){
     console.log(row);
      ...
      this.poolForm.limitParallelNumPerDevice = row.limitParallelNumPerDevice || 3;
      this.poolId = row.id
      this.dialogVisible = true;
   },
  ...
  unbindDeviceSend() {
    ...
    res => {
      if (res.code == 200) {
        ...
        this.resourcePoolBindedDeviceForm.orgId = Cookies.get("orgId");
   ...
   editParallel(row){
    ...
    sortableChange( obj, type ) {
      ...
      const matchingStr = {"editResourceName":"name","editPackageNum":"packageNum","devicesNum":"deviceNum","specificationDesc":"specification","durationDesc":"duration","bindTargetName":"bindTarget"};
      if (0 === type) {
    ...
    getResourcePoolBindedDevices(){
      this.loading2 = true;
      this.utils.http({
        url: this.isFlag?'/pool/specialized/search/device/page':'/pool/specialized/search/parallel/page',
        method: "POST",
  ...
  getSpecializedInfo(id){
    ...
    res => {
      ...
      if(res.code==200){
        ...
        let selectAppIds = []; 
        this.orgIdToDevice = res.data.allOrgList.filter(item => { return item.name && item.id }) || [];
        if (res.data.boundOgrList && res.data.boundOgrList.length>0) {
          this.selectOrgId = res.data.boundOgrList.map(item =>{return item.id}) || [];
        } else {
          this.selectOrgId = [];
        }
        this.appIdToDevice = res.data.appTreeList.filter(item => { return item.name && item.id }) || [];
