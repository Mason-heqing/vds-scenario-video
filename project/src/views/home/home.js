...
export default {
  ...
  data() {
    return {
      ...
      loading2:false,
      loading3:false,
      specialPackageTotalNum:0,//并行包总数
      specialPoolNum:0,//并行池数量
      specialPackageUseNum: 0,//当前并行包用量
      ...
      mayNotSpecialPackageTotalNum:0,//并行包未使用总数
      maySpecialPackageTotalNum:0,//并行包已使用总数
  ...
  mounted() {
    this.getTimeFn();
    this.getRealTimesDuration();//获取实时并行包数据
  ...
  methods: {
    ...
    getTopInfo(){
      ...
      res => {
        ...
        if (res.code == 200) {
           ...
           this.specialPackageUseNum = res.data.specialPackageUseNum||0;
    ...
    chartParallelPackageRate(){
      this.pieSpecialPackage = this.$echarts.init(document.getElementById('chartSpecialPackageRate'));
        ...
        series: [
          {
            ...
            data: [
              { value: this.maySpecialPackageTotalNum, name: '已使用',itemStyle: { color: '#00bfff' }},
              { value: this.mayNotSpecialPackageTotalNum, name: '未使用',itemStyle: { color: '#9acd32' }},
            ]
          }
        ]})
    },
    ...
    chartTimesPackAgePayRate(){
      this.pieDurationRate = this.$echarts.init(document.getElementById('chartRemainingTimes'));
    ...
    chartRemainingTimeLine(){
      this.columnarDuration = this.$echarts.init(document.getElementById('chartColumnar'));
      let option = {
    ...
    chartSpecialPoolOrgIdStatistics(){
      this.chartParallelPackageResourcePoolLine = this.$echarts.init(document.getElementById('chartParallelPackageResourcePool'));
      let orgName = [];
      let orgTlist = [];
    ...
    chartPayVideoStatistics(){
      this.chartpayVideoLine = this.$echarts.init(document.getElementById('chartVideo'));
      let option = {
        color:['#6cacde','rgb(200,200,169)'],
        tooltip: {
          trigger: 'axis',
